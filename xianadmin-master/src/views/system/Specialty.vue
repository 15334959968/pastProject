<template>
  <section class="level-manage">
    <el-button type="primary" icon="el-icon-plus" @click="add">新增专业</el-button>
    ...
    <el-dialog title="新增专业" :visible.sync="addVisible" :close-on-click-modal="false" append-to-body>
    ...
    <el-dialog title="专业详情" :visible.sync="detailVisible" append-to-body>
      <xa-dialog-form :model="detailForm" :disabled="true" label-width="80px"></xa-dialog-form>
    </el-dialog>

    <el-dialog title="专业编辑" :visible.sync="editVisible" :close-on-click-modal="false" append-to-body>
...
export default {
  ...
  methods: {
    ...
    del (id) {
      this.$servers.delSpecialty({ id }).then(() => {
    ...
    saveLevel (level, id) {
      ...
      this.$servers.saveSpecialty(option).then(data => {
