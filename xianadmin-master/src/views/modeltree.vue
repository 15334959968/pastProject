...
<script>
  ...
  export default {
    ...
    methods: {
        onloadModelsend(_this){
            _this.$root.$on("onloadModelsend",function (data) {
                if(data=='modeltree'){
                  setTimeout(function () {
                      _this.$parent.loadmodelkeyt="modeltreet";
                       console.log("this.$parent.modeltree");
                      _this.t.onWindowResize(threet);
                  },500)
                }
          })
        },
        checkrotertomodel(){
            this.onloadModelsend(this);
        },
    },
      beforeDestroy(){
           this.$parent.loadmodelkey="";
           this.$parent.loadmodelkeyt="";
      },
    mounted () {
       this.checkrotertomodel();
       this.$parent.loadmodelkey="modeltree";
