...
(function setMock(mockDir) {
  fs.readdirSync(mockDir).forEach(function (file) {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      ...
      let list=Object.keys(mock);
      list.forEach(function (apiKey) {
          let apiObject=mock[apiKey];
          app.use(apiObject.api, argv.proxy ? proxyMiddleware({target: 'http://' + argv.proxy}) : apiObject.response);
      })
    }
  });
})(mockDir);;
